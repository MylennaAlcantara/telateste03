...
export const Products = () => {
    ...
    useEffect(() => {
        async function fetchData (){
            const response = await fetch ("http://10.0.1.10:8088/cotacaoDetalhe/itens/10"); //Armazena em response as informações obtidas no fetch da url
            const data = await response.json();// converte para JSON as informações armazenadas em response
            setProducts(data);// Seto o estado de products com as informações ja transformadas em json
        }
        fetchData();
    }, []);
